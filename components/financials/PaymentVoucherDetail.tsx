...
import { PaymentVoucher, PaymentVoucherSettingsConfig, PaymentVoucherFieldConfig, ContactInfo } from '../../types';
...
const PaymentVoucherDetail: React.FC<PaymentVoucherDetailProps> = ({ voucherId, onBack }) => {
        ...
        const fetchVoucherData = async () => {
            ...
            try {
                const [voucherResponse, settingsResponse] = await Promise.all([
                    fetch(`${API_BASE_URL}payment_vouchers.php?id=${voucherId}`, { cache: 'no-cache', headers: { 'Accept': 'application/json' } }),
                    fetch(`${API_BASE_URL}settings.php?scope=paymentVoucher`, { cache: 'no-cache', headers: { 'Accept': 'application/json' } })
                ]);
    
                if (!voucherResponse.ok || !settingsResponse.ok) {
                    throw new Error(t('paymentVouchers.detail.notFound'));
                }
                ...
                const formattedVoucher: PaymentVoucher = {
                    id: String(data.id),
                    status: data.status,
                    date: data.date,
    ...
    if (error || !voucher || !settings) {
        return <div className="flex flex-col text-center items-center justify-center h-screen"><p className="font-bold text-red-600 text-lg mb-4">{t('paymentVouchers.detail.notFound')}</p><p className="text-gray-600">{error}</p></div>;
    }
        ...
        <div className="bg-gray-100 min-h-screen">
            ...
            <main className="p-4 sm:p-6 md:p-8">
                 <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg overflow-x-auto">
                    <div id="printable-voucher" className="p-8 sm:p-10 md:p-12 bg-white min-w-[800px]" dir={direction}>
                        ...
                        <div data-pdf-section="content" style={{ pageBreakInside: 'avoid' }}>
                            <div className="flex justify-between items-center mb-10">
                                ...
                                <div className={direction === 'rtl' ? 'text-left' : 'text-right'}>
                                    {findField('voucherNumber') && <p>{t(findField('voucherNumber')?.label as TranslationKey)}: <span className="font-mono">{voucher.id}</span></p>}
                                    {findField('date') && <p>{t(findField('date')?.label as TranslationKey)}: {voucher.date}</p>}
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                {renderDetailRow('paymentMethod', t(voucher.paymentMethod as TranslationKey))}
