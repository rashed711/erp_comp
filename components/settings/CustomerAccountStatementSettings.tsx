...
import { AccountStatementSettingsConfig, AccountStatementFieldConfig } from '../../types';
import { API_BASE_URL } from '../../services/api';
import { TranslationKey } from '../../i18n/translations';

const fieldTranslationKeys: Record<AccountStatementFieldConfig['key'], TranslationKey> = {
    contactInfo: 'settings.doc.field.customerData',
    statementDate: 'settings.doc.field.statementDate',
    openingBalance: 'settings.doc.field.openingBalance',
    totalDebit: 'settings.doc.field.totalDebit',
    totalCredit: 'settings.doc.field.totalCredit',
    closingBalance: 'settings.doc.field.closingBalance',
    ourCompanyInfo: 'settings.doc.field.companyData',
};
...
const CustomerAccountStatementSettings: React.FC = () => {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`${API_BASE_URL}settings.php?scope=customerStatement`, { cache: 'no-cache' });
                if (!response.ok) {
    ...
    const handleFieldLabelChange = (fieldKey: string, newLabel: string) => {
        ...
        setSettings({ ...settings, fields: updatedFields as AccountStatementFieldConfig[] });
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}settings.php?scope=customerStatement`, {
                method: 'POST',
                body: formData,
            });
            ...
            const updatedSettingsRes = await fetch(`${API_BASE_URL}settings.php?scope=customerStatement`);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md w-full">
            ...
            <div className="mb-6 pb-4 border-b">
                <h2 className="text-xl font-bold text-gray-800">{t('settings.doc.titleCustomerStatements')}</h2>
                <p className="text-sm text-gray-500 mt-1">{t('settings.doc.descriptionCustomerStatements')}</p>
            </div>
...
export default CustomerAccountStatementSettings;
