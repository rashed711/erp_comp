...
import { SalesInvoiceSettingsConfig, SalesInvoiceFieldConfig } from '../../types';
import { API_BASE_URL } from '../../services/api';
import { TranslationKey } from '../../i18n/translations';

const fieldTranslationKeys: Record<SalesInvoiceFieldConfig['key'], TranslationKey> = {
    customerInfo: 'settings.doc.field.invoiceTo',
    invoiceNumber: 'settings.doc.field.invoiceNumber',
...
const SalesInvoiceSettings: React.FC = () => {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`${API_BASE_URL}settings.php?scope=salesInvoice`, { cache: 'no-cache' });
                if (!response.ok) {
                    throw new Error(t('settings.doc.titleSalesInvoices') + ': ' + t('common.error'));
                }
    ...
    const handleFieldLabelChange = (fieldKey: string, newLabel: string) => {
        ...
        setSettings({ ...settings, fields: updatedFields as SalesInvoiceFieldConfig[] });
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}settings.php?scope=salesInvoice`, {
                method: 'POST',
                body: formData,
            });
            ...
            const updatedSettingsRes = await fetch(`${API_BASE_URL}settings.php?scope=salesInvoice`);
