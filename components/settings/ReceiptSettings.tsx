...
import { ReceiptSettingsConfig, ReceiptFieldConfig } from '../../types';
import { API_BASE_URL } from '../../services/api';
import { TranslationKey } from '../../i18n/translations';

const fieldTranslationKeys: Record<ReceiptFieldConfig['key'], TranslationKey> = {
    customerInfo: 'settings.doc.field.receivedFrom',
    receiptNumber: 'settings.doc.field.receiptNumber',
...
const ReceiptSettings: React.FC = () => {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`${API_BASE_URL}settings.php?scope=receipt`, { cache: 'no-cache' });
                if (!response.ok) {
                    throw new Error(t('settings.doc.titleReceipts') + ': ' + t('common.error'));
                }
    ...
    const handleFieldLabelChange = (fieldKey: string, newLabel: string) => {
        ...
        setSettings({ ...settings, fields: updatedFields as ReceiptFieldConfig[] });
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}settings.php?scope=receipt`, {
                method: 'POST',
                body: formData,
            });
            ...
            const updatedSettingsRes = await fetch(`${API_BASE_URL}settings.php?scope=receipt`);
