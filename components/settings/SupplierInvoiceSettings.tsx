...
import { SupplierInvoiceSettingsConfig, SupplierInvoiceFieldConfig } from '../../types';
import { API_BASE_URL } from '../../services/api';
import { TranslationKey } from '../../i18n/translations';

const fieldTranslationKeys: Record<SupplierInvoiceFieldConfig['key'], TranslationKey> = {
    supplierInfo: 'settings.doc.field.invoiceFrom',
    supplierInvoiceNumber: 'settings.doc.field.supplierInvoiceNumber',
    invoiceDate: 'settings.doc.field.invoiceDate',
    dueDate: 'settings.doc.field.dueDate',
};

const SupplierInvoiceSettings: React.FC = () => {
    const { t } = useI18n();
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            setError(null);
            try {
                const response = await fetch(`${API_BASE_URL}settings.php?scope=supplierInvoice`, { cache: 'no-cache' });
                if (!response.ok) {
                    throw new Error(t('settings.doc.titlePurchaseInvoices') + ': ' + t('common.error'));
                }
    ...
    const handleFieldLabelChange = (fieldKey: string, newLabel: string) => {
        ...
        setSettings({ ...settings, fields: updatedFields as SupplierInvoiceFieldConfig[] });
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}settings.php?scope=supplierInvoice`, {
                method: 'POST',
                body: formData,
            });
            ...
            const updatedSettingsRes = await fetch(`${API_BASE_URL}settings.php?scope=supplierInvoice`);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md w-full">
            ...
            <div className="mb-6 pb-4 border-b">
                <h2 className="text-xl font-bold text-gray-800">{t('settings.doc.titlePurchaseInvoices')}</h2>
                <p className="text-sm text-gray-500 mt-1">{t('settings.doc.descriptionPurchaseInvoices')}</p>
            </div>
            ...
            <form className="space-y-6" onSubmit={handleSave}>
                     ...
                     <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        {settings?.fields.map(field => (
                            <ConfigurableField
                                key={field.key}
                                fieldKey={field.key}
                                label={t(fieldTranslationKeys[field.key as keyof typeof fieldTranslationKeys])}
                ...
                <SettingsCard title={`3. ${t('settings.doc.sectionFooter')}`} description={t('settings.doc.sectionFooterDescription')}>
                    <div>
                        <label htmlFor="defaultTerms" className="block text-sm font-medium text-gray-700 mb-1">{t('settings.doc.defaultTerms')}</label>
                        <textarea
                            id="defaultTerms"
                            rows={5}
