...
import { Supplier, Product, DocumentItem, SupplierInvoiceSettingsConfig, CurrencySettingsConfig, Currency } from '../../types';
import { getSupplierInvoiceSettings, getSuppliers as getMockSuppliers, getProducts as getMockProducts, getCurrencySettings } from '../../services/mockApi';
...
const CreateSupplierInvoice: React.FC<CreateSupplierInvoiceProps> = ({ onBack }) => {
        ...
        const initialLoad = async () => {
            ...
            try {
                const results = await Promise.allSettled([
                    fetch(`${API_BASE_URL}suppliers.php`, { cache: 'no-cache', headers: { 'Accept': 'application/json' } }),
                    fetch(`${API_BASE_URL}products.php`, { cache: 'no-cache', headers: { 'Accept': 'application/json' } })
                ]);
                
                const [supplierResult, productResult] = results;
                const fetchErrors: { name: string; text: string }[] = [];

                if (supplierResult.status === 'fulfilled') {
                    const response = supplierResult.value;
                    const text = await response.text();
                    if (response.ok) {
                        try {
                            setSuppliers(JSON.parse(text));
                        } catch (e) {
                            fetchErrors.push({ name: 'suppliers.php', text });
                    ...
                    fetchErrors.push({ name: 'suppliers.php', text: supplierResult.reason.message });
                    setSuppliers(getMockSuppliers());
                }
    ...
    const handleSupplierNameBlur = () => {
        ...
        const isValidSupplier = suppliers.some(s => s.name === supplierInput);
        if (!isValidSupplier) {
            setSupplierInput('');
            setSelectedSupplier(null);
        }
            ...
            if (err instanceof SyntaxError) {
                detailedError = (
                    <div>
                        ...
                        <p className="mt-2 text-sm">حدث خطأ فادح (Fatal Error) في ملف PHP على الخادم (`supplier_invoices.php`).</p>
                                ...
                                <tbody>
                                    {items.map((item) => (
                                        <tr key={item.id} className="border-b">
                                            <td className="p-2">
                                                <input list="products-list" placeholder={t('docCreate.itemPlaceholder')} className="w-full p-2 border border-gray-200 rounded-md" value={item.productName || ''} onChange={(e) => handleItemChange(item.id, 'productName', e.target.value)} onBlur={() => handleProductNameBlur(item.id)} />
                                            </td>
                                            <td className="p-2">
                                                <input placeholder={t('docCreate.descriptionPlaceholder')} className="w-full p-2 border border-gray-200 rounded-md" value={item.description} onChange={(e) => handleItemChange(item.id, 'description', e.target.value)} />
                                            </td>
                          ...
                          <div className="w-full lg:w-1/2">
                              <div className="bg-gray-50 p-6 rounded-lg border space-y-4">
                                  ...
                                  <div className="flex justify-between items-center">
                                      ...
                                      <span className="font-medium text-gray-800">{formatCurrency(subtotal, selectedCurrency ? t(selectedCurrency.symbol as TranslationKey) : '', false)}</span>
                                  </div>
                                  <div className="flex justify-between items-center">
